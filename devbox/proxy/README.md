...
Puts `llama-server` and friends on `https://<name>.oeilvert.org`, behind a
GitHub login, without either of them knowing about it.
...
Two warnings. This is **zone-wide**: every other origin behind `oeilvert.org`
must already present a certificate Cloudflare trusts, or it breaks when the mode
changes. And Full alone is not a substitute — it accepts any certificate the
origin offers, including a forged one, which is the whole reason not to use a
self-signed certificate here (`docs/adr/0002`).
...
| Scope | Permission |
|---|---|
| Zone → `oeilvert.org` | SSL and Certificates → Edit |

For `sync-cloudflare.sh`:

| Scope | Permission |
|---|---|
| Zone → `oeilvert.org` | Zone → Read |
| Zone → `oeilvert.org` | DNS → Edit |
...
This writes `~/.config/devbox-proxy/certs/origin.{pem,key}` for
`*.oeilvert.org` plus the apex, valid for fifteen years.
...
`https://llama.oeilvert.org` for you and anyone in the `acme` GitHub org.
...
`<label>.oeilvert.org` that the file no longer mentions, and only ones that are
proxied — `anietta.oeilvert.org` is a grey-cloud record and is left alone.
...
**The front door.** Open `https://llama.oeilvert.org` in a browser. Expect a
...
curl -sk -o /dev/null -w '%{http_code}\n' \
    --resolve 'llama.oeilvert.org:443:[2405:6581:8580:310::153]' \
    https://llama.oeilvert.org/
```
